    ...
    <div className="pointer-events-none absolute inset-0 hidden lg:block" aria-hidden>
      {STAGE_PETALS.map((p, i) => (
        <Petal
          ...
          className="absolute"
          style={{ top: p.top, bottom: p.bottom, left: p.left, right: p.right, transform: `rotate(${p.rotate}deg)` }}
        />
      ))}
    </div>
  );
      ...
      >
        {(showBack || showProgress) && (
          <div className="flex items-center justify-between px-6 pt-5">
            ...
            {showProgress ? <ProgressDots total={progressTotal} current={progressCurrent} /> : <span />}
            <span className="h-9 w-9 shrink-0" />
          </div>
        )}

        <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-6 pb-8 pt-6">{children}</div>
      </div>
    </div>
  );
}
